...
void CAN_RxFrameInit(CAN_Type *CAN, uint8_t MBn, uint32_t ID)
{
	...
	CAN->RAMn[MBn*MB_SZIE + 0] = MB_Rx_RANSWER << 24;
...
void CAN0_ORed_0_15_MB_IRQHandler()
{
	...
	if(mbs_recept_flag & (1 << count))
	{
		...
		RxDATA[0]   = CAN0->RAMn[count * MB_SZIE + 2];
		RxDATA[1]   = CAN0->RAMn[count * MB_SZIE + 3];
		RxTIMESTAMP = CAN0->RAMn[count * MB_SZIE + 0] & 0xFFFF;

		//LPUART1_transmit_string("recieve data success!\n");
	...
	{
		//LPUART1_transmit_string("transfer data success!\n");
		/*do nothing*/
	}
